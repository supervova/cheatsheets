...
4. И вообще никаких `new`.
5. Остерегаться случайного преобразования типов.
6. Использовать тройное равно для сравнения.
7. Функции: JSDoc-комментарии и параметры по умолчанию.
8. Заканчивать switch default'ом.
9. Не использовать eval().
10. Не использовать параллельные массивы.
11. Не хранить длину массива в переменной.
12. `eventListener` вместо `on`-функций
13. Соглашение об именовании.
14. Использовать директиву 'use strict mode';
15. Всегда придерживаться одного стиля кодирования.
...
Строки, числа и логические типы данных можно преобразовывать, если надо, и сравнивать. Объекты сравнивать между собой или с примитивами нельзя. eslint: no-new-wrappers

```javascript
const y = new String('John'); // ⛔️
```

#### 4) И вообще НИКАКИХ new Object(), String(), Number(), Boolean(), Array() и Function()

Это усложняет код и замедляет его выполнение.

Исключение — `new RegExp()`, которое используется, когда надо создать регулярное выражение «на лету» из динамически сгенерированной строки.
...
#### 7) Функции всегда надо комментировать по стандарту JSDoc и по возможности использовать параметры по умолчанию
...
#### 11) Не хранить длину массива в специальной переменной
...
#### 15) Всегда придерживаться общего стиля кодирования
